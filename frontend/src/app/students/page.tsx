      ...
      {!loading && students.length > 0 && (
        <div className="space-y-3">
          {students.map((s) => {
            ...
            const paidPct = sum ? Math.min(100, Math.round((sum.paidAmount / sum.totalFees) * 100)) : 0;
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {s.age && (
                          <p className="text-xs text-slate-500 mt-0.5">Age {s.age}</p>
                        )}
                      </div>
                    </div>
                ...
                {isExpanded && sum && (
                  <div className="border-t border-slate-100 bg-slate-50 px-5 py-4">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4">
                      {[
                        { label: "Total Fees", value: sum.totalFees, color: "text-slate-700" },
                        { label: "Paid", value: sum.paidAmount, color: "text-emerald-700" },
                        { label: "Pending", value: sum.pendingAmount, color: sum.pendingAmount > 0 ? "text-red-600" : "text-slate-400" },
                      ].map(({ label, value, color }) => (
                        <div key={label} className="bg-white border border-slate-100 rounded-lg p-3">
                          <p className="text-xs text-slate-500 mb-1">{label}</p>
                          <p className={`text-base font-semibold ${color}`}>₹{value.toLocaleString("en-IN")}</p>
                        </div>
                      ))}
                    </div>
                      ...
                      <div className="flex gap-2 mb-4">
                        <div className="relative flex-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setPayAmountInput((p) => ({ ...p, [s._id]: e.target.value }))
                            }
